...
# Get the absolute path to the project's root directory (studypal/studypal)
working_dir = os.path.dirname(os.path.abspath(__file__))
parent_dir = os.path.dirname(working_dir)

# Construct the path to the virtual environment's torch lib
# This assumes the .venv is in the parent directory of the 'src' directory
venv_torch_lib_path = os.path.join(parent_dir, '.venv', 'Lib', 'site-packages', 'torch', 'lib')

# Prepend the venv torch lib path to the system PATH and add it to the DLL search path
if os.path.exists(venv_torch_lib_path):
    os.environ['PATH'] = venv_torch_lib_path + os.pathsep + os.environ['PATH']
    if sys.version_info >= (3, 8):
        os.add_dll_directory(venv_torch_lib_path)
